    ...
    def get_weather(self):
        try:
            if latitude is None and longitude is None:
                # get location
                location_req_url = "http://api.ipstack.com/%s?access_key=%s&format=1" % (
                    self.get_ip(), ip-stack_key)
                r = requests.get(location_req_url)
                location_obj = json.loads(r.text)
                lat = location_obj['latitude']
                lon = location_obj['longitude']
                location2 = "%s, %s" % (location_obj['city'], loca-tion_obj['region_code'])
                # get weather
                weather_req_url = "https://api.openweathermap.org/data/2.5/onecall?lat=%s&lon=%s&appid=%s&units=metric" % (
                    lat, lon, weather_api_token)
                # weather_req_url = "https://api.darksky.net/forecast/%s/%s,%s?lang=%s&units=%s" % (weath-er_api_token,lat, lon, weather_lang, weather_unit)
                
            else:
                location2=""
                # get weather
                weather_req_url="https://api.openweathermap.org/data/2.5/onecall?lat=%s&lon=%s&appid=%s&units=metric" % (
                    latitude, longitude, weather_api_token)
                # weather_req_url = "https://api.darksky.net/forecast/%s/%s,%s?lang=%s&units=%s" % (weath-er_api_token, latitude, longitude,weather_lang, weather_unit)
            ...
            temperature2="%s%s" % (
                str(int(weather_obj['current']['temp'])), degree_sign)
    ...
    def get_headlines(self):
        try:
            ...
            if news_country_code == None:
                headlines_url="https://newsapi.org/v2/top-headlines?country=us&apiKey=%s" % news_api
            else:
                headlines_url="https://newsapi.org/v2/top-headlines?country=%s&apiKey=%s" % (news_country_code, news_api)
...
class Calendar(Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.calendarLbl=Label(self, text=self.title, font=(
            'Helvetica', medium_text_size), fg="white", bg="black")
